...
class LLaDALlamaBlock(LLaDABlock):
    ...
    def forward(
        ...
        if layer_past is not None and use_cache:
            ...
            future_k, future_v = future_cache
        ...
        # in a KV reuse step. 
            ...
            # print("prev pos shape:", prv_position.shape)
            ...
            # cache for far away mask tokens:
            if cache_future_idx is not None:
                future_pos = cache_future_idx.unsqueeze(-1).expand(B, -1, D)
                fut_k = ori_k[future_pos].view(B, -1, D)
                fut_v = ori_v[future_pos].view(B, -1, D)
                past_k = torch.cat([past_k, fut_k], dim=1)
                past_v = torch.cat([past_v, fut_v], dim=1)
