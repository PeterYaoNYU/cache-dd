...
class LLaDAEvalHarness(LM):
    def __init__(
        ...
        **kwargs,
        ...
        self.cache_reload_step = cache_reload_step
        self.far_gap = far_gap
    ...
    def generate_until(self, requests: list[Instance]):
        ...
        out = [None] * len(ds)                       # keep original order
        batch_q, batch_until, batch_idx = [], [], []
        ...
        def _flush():
            ...
            prompt_batch = torch.nn.utils.rnn.pad_sequence(
                batch_q, batch_first=True,
                padding_value=self.tokenizer.pad_token_id
            ).to(self.device)                                     # ★
            # ----- call your generator ---------------------------------------
            gen = generate(
                self.model, self.tokenizer, prompt_batch,
                steps=self.steps, gen_length=self.gen_length,
                block_length=self.block_length,
                temperature=0, cfg_scale=self.cfg,
                remasking=self.remasking, mask_id=self.mask_id,
                enable_cache=True, cache_reloading_step=self.cache_reload_step,
                far_gap=self.far_gap,
            )                                                     # ★
            # -------- post-process each sequence in the mini-batch -----------
            for loc, gen_ids, stop_tokens in zip(
                batch_idx, gen, batch_until
            ):
                text = self.tokenizer.decode(
                    gen_ids[prompt_batch.shape[1] :],             # skip prompt
                ...
                out[loc] = text
            # -------- reset buffers ------------------------------------------
            batch_q.clear(); batch_until.clear(); batch_idx.clear()
            if self.accelerator is not None:
                self.accelerator.wait_for_everyone()

        # main loop
        for i, row in enumerate(tqdm(ds, desc="Generating...")):
            batch_q.append(row["question"].to(self.device))
            batch_until.append(row["until"])
            batch_idx.append(i)
            if len(batch_q) == self.batch_size:
                _flush()
        _flush()   # flush leftovers
        return out
