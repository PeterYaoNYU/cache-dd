...
for i in "${!TASKS_ARRAY[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES=3,4,5,6,7 accelerate launch eval_dream.py --model dream \
        --model_args pretrained=${model},max_new_tokens=${LENGTH_ARRAY[$i]},diffusion_steps=${LENGTH_ARRAY[$i]},add_bos_token=true,temperature=${TEMP_ARRAY[$i]},top_p=0.95,use_cache=true,cache_type="decoded",cache_steps=4,shift_type="un" \
        --tasks ${TASKS_ARRAY[$i]} \
